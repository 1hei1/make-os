...
void HariMain(void)
{
	...
	unsigned char layer_content_ads[80];
	sprintf(layer_content_ads,"NUMBER1OKOK");
	draw_mouse(0,0,8,8,8,fontpk,&mousess);//绘制鼠标形状
	struct layer_content mtest;
	mtest.x=0;
	mtest.y=0;
	mtest.ads=layer_content_ads;                      //在赋值问题上存储地址的指针声明为int型，对应存放字符串的数组可以是任意类型，但指向该数组的指针按照需求而定不能随意更改
	...
	windows_set(back,&mtest,60,80,100,30,2);
	//windows_set(mouse,mousess,16,16,30,90,2);
	//windows_set(test,&mtest,80,100,100,80,2);
			...
			if (64 - (jianpan.free) != 0) {
				...
				if (shubiao1.q < 320) {
					if (date == 0xfa) {
						...
						shubiao1.free++;
						oi_sti();
						drawnormal(mousex, mousey,mousex+16,mousey+16,320, 0xa0000, 6);
						...
						drawnormal(mousex,mousey,mousex+16,mousey+16,320,0xa0000,10);
