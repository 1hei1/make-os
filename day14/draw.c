...
void init_palette(void) {
	static unsigned char table_rgb[16 * 3] = {
		0x00,0x00,0x00,//black0
		0xff,0x00,0x00,//red1
		0x00,0xff,0x00,//green2
		0xff,0xff,0x00,//yellow3
		0x00,0x00,0xff,//qingse4
		0xff,0x00,0xff,//puple5
		0x00,0xff,0xff,//light blue6
		0xff,0xff,0xff,//white7
		0xc6,0xc6,0xc6,//light black8
		0x84,0x00,0x00,//dark red9
		0x00,0x84,0x00,//drak green10
		0x84,0x84,0x00,//drak yellow11
		0x00,0x00,0x84,//drak green12
		0x84,0x00,0x84,//light drak blue13
		0x00,0x84,0x84,//drak light black14
		0x84,0x84,0x84//drak pople15
	};
...
void set_palette(int start, int end, unsigned char* rgb) {
	...
	for (i = start; i <= end; i++) {
		oi_8out(0x03c9, rgb[0]/4);
		oi_8out(0x03c9, rgb[1]/4);
		oi_8out(0x03c9, rgb[2]/4);
		rgb += 3;
	}
...
void drawnormal_font_scr(int x, int y,int xsize, char font, char* vram,int c) {
	int n;
	extern char fontpk[4096];
	char *p;
	p=fontpk+font*16;
	...
	if ((d & 0x40) != 0){
	vram[x + (y+n)*xsize+1] = c;
	}
	if ((d & 0x20) != 0){
	vram[x + (y+n)*xsize+2] = c;
	}
	if ((d & 0x10) != 0){
	vram[x + (y+n)*xsize+3] = c;
	}
	if ((d & 0x08) != 0){
	vram[x + (y+n)*xsize+4] = c;
	}
	if ((d & 0x04) != 0){
	vram[x + (y+n)*xsize+5] = c;
	}
	if ((d & 0x02) != 0){
	vram[x + (y+n)*xsize+6] = c;
	}
	if ((d & 0x01) != 0){
	vram[x + (y+n)*xsize+7] = c;
	}
	}
	return;
}/*绘制字符*/
...
for (;*ads_src!=0;ads_src++) {
	drawnormal_font_scr(x, y,xsize, *ads_src, ads, color);
	x += 8;
	if (x >= xsize-8) {
		y += 16;
		x = 0;
...
void draw_mouse(int setx, int sety, int a, int b, int c, char* fontpk,unsigned char *ads) {
	//char* ads = 0xa0000;
	//char m[256];
	char mouse[16][16] = {
	"**************..",
	...
	for (y = 0; y < 16; y++) {
		for (x = 0; x < 16; x++) {
			if (mouse[y][x] == '*') {
				ads[setx+sety*320+n+j] = a;
				
			}
			if (mouse[y][x] == '0') {
				ads[setx+sety*320+n+j] = b;
				
			}
			if (mouse[y][x] == '.') {
				ads[setx+sety*320+n+j] = c;
...
void draw_rout(int x, int y,char *ads) {
	char rout[16][16] = {
"**************..",
"*00000000000*...",
"*0000000000*....",
