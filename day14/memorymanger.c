...
void windows_set(struct windows_content*std,char *ads,int xsize,int ysize,int x,int y){
	std->content_ads = ads;
...
/*void windows_refresh(struct manger_windows *mws){
	...
	for (i=0;i<=mws->top;i++){
		...
		char m[10];
		sprintf(m,"NO");
		struct layer_backs *lay=std->layer_backs;
		for (j=0;j<std->layer_backs->blocks;j++){
			drawnormal((lay->content[j]->x),(lay->content[j]->y),lay->content[j]->x1,lay->content[j]->y1,320,0xa0000+std->x+std->y*320,lay->content[j]->c);
		};
		
		//drawnormal(std->x,std->y,std->x+std->xsize,std->y+std->ysize,320,0xa0000,std->color);
		drawnormal_src(std->content_ads->x,std->content_ads->y,std->xsize,0xa0000+std->x+(std->y)*320,8,std->content_ads->ads);
...
void window_draw(struct windows_content *std,char * start){
	int x= std->x;
	int y= std->y;
	int xsize = std->xsize;
	int ysize = std->ysize;
	...
	for (y=0;y<ysize;y++){
		for (x=0;x<xsize;x++){
			start[(std->x)+x+(y+std->y)*320]=std->content_ads[i];
			i++;
		}
...
void window_draw_map(struct manger_windows*mws,char *map){
	int i=1;
	for(i=1;i<=mws->top;i++){
		struct window_content * std=mws->window_contentd_ads[i];
		window_draw(std,map);
	};
